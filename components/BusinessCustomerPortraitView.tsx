...
import { PortraitConfigView } from './PortraitConfigView';
import { PortraitDashboardView } from './PortraitDashboardView';
import { PortraitDetailsView } from './PortraitDetailsView';
import { SettingsIcon, BarChartIcon, SidebarCloseIcon, SidebarOpenIcon, ListIcon, XIcon } from './Icons';

export const BusinessCustomerPortraitView: React.FC = () => {
    const [tabs, setTabs] = useState<{ id: string, label: string }[]>([{ id: 'dashboard', label: '商客装维画像' }]);
    const [activeTabId, setActiveTabId] = useState<string>('dashboard');
    const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(false);
    const [contextMenu, setContextMenu] = useState<{ x: number, y: number, tabId: string } | null>(null);

    const sidebarItems = [
        { id: 'dashboard', label: '商客装维画像概览', icon: <BarChartIcon /> },
        { id: 'details', label: '商客装维画像明细', icon: <ListIcon /> },
        { id: 'config', label: '画像配置', icon: <SettingsIcon /> },
    ];

    const handleSidebarClick = (item: { id: string, label: string }) => {
        if (!tabs.find(t => t.id === item.id)) {
            const label = item.id === 'dashboard' ? '商客装维画像' : item.label;
        ...
        <div className="flex flex-1 overflow-hidden h-full">
            ...
            <div className={`${isSidebarCollapsed ? 'w-[53px]' : 'w-48'} bg-transparent border border-blue-500/30 mr-2 transition-all duration-500 ease-in-out flex flex-col shadow-[0_0_15px_rgba(0,0,0,0.3)]`}>
                <div className={`h-[35px] flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between px-3'} border-b border-blue-500/20 bg-transparent shrink-0`}> 
                    {!isSidebarCollapsed && <span className="text-blue-100 font-bold tracking-wider text-[12px] whitespace-nowrap">商客装维画像</span>}
                            ...
                            <div key={tab.id} className={`flex-1 flex flex-col h-full ${isActive ? '' : 'hidden'}`}>
                                {tab.id === 'config' ? (
                                    ...
                                    <PortraitDashboardView 
                                        ...
                                        onNavigateToDetails={() => handleSidebarClick({ id: 'details', label: '商客装维画像明细' })}
                                    />
                                )}
                            </div>
                        );
