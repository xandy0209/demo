...
import { TrafficOverlimitAnalysisRecord, TrafficOverlimitDetailRecord } from '../types';
import { generateTrafficOverlimitMockData, generateTrafficOverlimitDetailMockData, INNER_MONGOLIA_CITIES } from '../constants';
import { StyledInput, StyledButton, StyledSelect } from './UI';
import { SearchIcon, DownloadIcon, XIcon, BarChartIcon, ClockIcon, RefreshCwIcon } from './Icons';
...
export const TrafficOverlimitAnalysisView: React.FC = () => {
    ...
    const [detailView, setDetailView] = useState<{ isOpen: boolean, type: 'zero' | 'high' | null, record: TrafficOverlimitAnalysisRecord | null }>({
        isOpen: false,
        type: null,
        record: null
    });
    
    const [detailData, setDetailData] = useState<TrafficOverlimitDetailRecord[]>([]);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            setData(generateTrafficOverlimitMockData(100));
            setLoading(false);
        }, 800);
    ...
    const handleOpenDetail = (record: TrafficOverlimitAnalysisRecord, type: 'zero' | 'high') => {
        setDetailView({ isOpen: true, type, record });
        setDetailPagination(prev => ({ ...prev, currentPage: 1 }));
        setDetailLoading(true);
        setTimeout(() => {
            setDetailData(generateTrafficOverlimitDetailMockData(50, type));
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-transparent border border-blue-500/30 shadow-[inset_0_0_20px_rgba(0,133,208,0.1)] relative">
            ...
            <div className="flex-1 overflow-auto bg-transparent scrollbar-thin">
                <table className="w-full text-left text-xs whitespace-nowrap border-separate border-spacing-0">
                    <thead className="sticky top-0 bg-[#0c2242] text-blue-200 z-10 shadow-sm">
                        <tr>
                            <th className="p-3 border-b border-blue-500/20 font-medium">时间</th>
                            ...
                            <th className="p-3 border-b border-blue-500/20 font-medium text-center">零流量业务数</th>
                            <th className="p-3 border-b border-blue-500/20 font-medium text-center">零流量业务占比 (%)</th>
                            <th className="p-3 border-b border-blue-500/20 font-medium text-center">高带宽占用超限业务数</th>
                            <th className="p-3 border-b border-blue-500/20 font-medium text-center">高带宽占用超限业务占比 (%)</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td colSpan={7} className="p-10 text-center">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <span className="text-blue-400 text-sm">正在采集分析数据...</span>
                                    </div>
                                </td>
                            </tr>
                            ...
                            paginatedData.map((item) => (
                                <tr key={item.id} className="hover:bg-[#1e3a5f]/60 transition-colors border-b border-blue-500/10 last:border-0">
                                    ...
                                    <td className="p-3 border-b border-blue-500/10 text-center">
                                        <button 
                                            onClick={() => handleOpenDetail(item, 'zero')}
                                            className="text-blue-400 hover:underline font-mono"
                                        >
                                            {item.zeroTrafficCount}
                                        </button>
                                    </td>
                                    <td className="p-3 border-b border-blue-500/10 text-center font-mono">{item.zeroTrafficRatio}%</td>
                                    <td className="p-3 border-b border-blue-500/10 text-center">
                                        <button 
                                            onClick={() => handleOpenDetail(item, 'high')}
                                            className="text-orange-400 hover:underline font-mono"
                                        >
                                            {item.highBandwidthCount}
                                        </button>
                                    </td>
                                    <td className="p-3 border-b border-blue-500/10 text-center font-mono">{item.highBandwidthRatio}%</td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan={7} className="p-10 text-center text-blue-300/50 border-b border-blue-500/10 italic">暂无统计数据</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            {detailView.isOpen && (
                <div className="absolute inset-0 z-[50] flex items-center justify-center p-4 sm:p-8">
                    <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setDetailView({ isOpen: false, type: null, record: null })}></div>
                    <div className="relative w-full h-full bg-[#0b1730] border border-blue-500/40 rounded-lg shadow-[0_0_40px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden animate-[fadeIn_0.2s_ease-out]">
                        {/* Modal Header */}
                        <div className="py-2 px-4 border-b border-blue-500/20 flex items-center justify-between bg-[#0c2242]/50">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-white font-medium text-sm">
                                    {detailView.type === 'zero' ? '零流量业务详情' : '高带宽占用超限业务详情'} - {detailView.record?.city} ({detailView.record?.time})
                                </h3>
                            </div>
                            <button 
                                onClick={() => setDetailView({ isOpen: false, type: null, record: null })}
                        ...
                        <div className="flex-1 overflow-hidden flex flex-col p-6">
                            <div className="flex-1 overflow-auto bg-transparent scrollbar-thin border border-blue-500/20 rounded-sm">
                                <table className="w-full text-left text-[11px] whitespace-nowrap border-separate border-spacing-0 min-w-[2000px]">
                                    <thead className="sticky top-0 bg-[#0c2242] text-blue-200 z-10 shadow-sm">
                                        <tr>
                                            ...
                                            <th className="p-3 border-b border-blue-500/20 font-medium">上行流量</th>
                                            <th className="p-3 border-b border-blue-500/20 font-medium">下行流量</th>
                                            <th className="p-3 border-b border-blue-500/20 font-medium">上行速率</th>
                                            <th className="p-3 border-b border-blue-500/20 font-medium">下行速率</th>
                                            <th className="p-3 border-b border-blue-500/20 font-medium">上行利用率</th>
                                            <th className="p-3 border-b border-blue-500/20 font-medium">下行利用率</th>
                                            <th className="p-3 border-b border-blue-500/20 font-medium">最后上线时间</th>
                                        </tr>
                                    </thead>
                                    <tbody className="text-white">
                                        {detailLoading ? (
                                            ...
                                            paginatedDetailData.map((item) => (
                                                <tr key={item.id} className="hover:bg-[#1e3a5f]/60 transition-colors border-b border-blue-500/10 last:border-0">
                                                    ...
                                                    <td className="p-3 border-b border-blue-500/10 font-mono">{item.upstreamTraffic}</td>
                                                    <td className="p-3 border-b border-blue-500/10 font-mono">{item.downstreamTraffic}</td>
                                                    <td className="p-3 border-b border-blue-500/10 font-mono">{item.upstreamRate}</td>
                                                    <td className="p-3 border-b border-blue-500/10 font-mono">{item.downstreamRate}</td>
                                                    <td className="p-3 border-b border-blue-500/10 font-mono">{item.upstreamUtilization}</td>
                                                    <td className="p-3 border-b border-blue-500/10 font-mono">{item.downstreamUtilization}</td>
                                                    <td className="p-3 border-b border-blue-500/10 font-mono">{item.lastOnlineTime}</td>
                                                </tr>
                                            ))
                                        ) : (
                                            <tr>
                                                <td colSpan={17} className="p-10 text-center text-blue-300/50 border-b border-blue-500/10 italic">暂无详情数据</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                        ...
                        {/* Modal Footer */}
                        <div className="h-[44px] border-t border-blue-500/20 flex items-center justify-between px-6 bg-[#0b1730]/60">
                            <div className="flex items-center gap-4">
